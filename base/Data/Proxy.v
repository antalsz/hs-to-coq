...
Local Definition Ord__Proxy_op_zgze__ {inst_s} : (Proxy inst_s) -> (Proxy
                                                 inst_s) -> bool :=
  fun x y => _GHC.Base./=_ (Ord__Proxy_compare x y) Lt.

Local Definition Ord__Proxy_op_zl__ {inst_s} : (Proxy inst_s) -> (Proxy
                                               inst_s) -> bool :=
  fun x y => _GHC.Base.==_ (Ord__Proxy_compare x y) Lt.

Local Definition Ord__Proxy_op_zlze__ {inst_s} : (Proxy inst_s) -> (Proxy
                                                 inst_s) -> bool :=
  fun x y => _GHC.Base./=_ (Ord__Proxy_compare x y) Gt.

Local Definition Ord__Proxy_max {inst_s} : (Proxy inst_s) -> (Proxy
                                           inst_s) -> (Proxy inst_s) :=
  fun x y => if Ord__Proxy_op_zlze__ x y : bool then y else x.

Local Definition Ord__Proxy_min {inst_s} : (Proxy inst_s) -> (Proxy
                                           inst_s) -> (Proxy inst_s) :=
  fun x y => if Ord__Proxy_op_zlze__ x y : bool then x else y.

Program Instance Ord__Proxy {s} : GHC.Base.Ord (Proxy s) := fun _ k =>
...
Program Instance Monoid__Proxy {s} : GHC.Base.Monoid (Proxy s) := fun _ k =>
...
Local Definition Monad__Proxy_op_zgzg__ : forall {a} {b},
                                            Proxy a -> Proxy b -> Proxy b :=
  fun {a} {b} => _GHC.Base.*>_.

Local Definition Monad__Proxy_op_zgzgze__ : forall {a} {b},
...
Program Instance Monad__Proxy : GHC.Base.Monad Proxy := fun _ k =>
