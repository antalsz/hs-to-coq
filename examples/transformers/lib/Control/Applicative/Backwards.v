...
Local Definition Eq___Backwards_op_zeze__ {inst_f} {inst_a} `{Eq1 inst_f} `{Eq_
  inst_a}
   : (Backwards inst_f inst_a) -> (Backwards inst_f inst_a) -> bool :=
  eq1.

Local Definition Eq___Backwards_op_zsze__ {inst_f} {inst_a} `{Eq1 inst_f} `{Eq_
  inst_a}
   : (Backwards inst_f inst_a) -> (Backwards inst_f inst_a) -> bool :=
  fun x y => negb (Eq___Backwards_op_zeze__ x y).

Program Instance Eq___Backwards {f} {a} `{Eq1 f} `{Eq_ a}
   : Eq_ (Backwards f a) :=
  fun _ k =>
    k {| op_zeze____ := Eq___Backwards_op_zeze__ ;
         op_zsze____ := Eq___Backwards_op_zsze__ |}.

Local Definition Ord__Backwards_compare {inst_f} {inst_a} `{Ord1 inst_f} `{Ord
  inst_a}
   : (Backwards inst_f inst_a) -> (Backwards inst_f inst_a) -> comparison :=
  compare1.

Local Definition Ord__Backwards_op_zg__ {inst_f} {inst_a} `{Ord1 inst_f} `{Ord
  inst_a}
   : (Backwards inst_f inst_a) -> (Backwards inst_f inst_a) -> bool :=
  fun x y => _==_ (Ord__Backwards_compare x y) Gt.

Local Definition Ord__Backwards_op_zgze__ {inst_f} {inst_a} `{Ord1 inst_f} `{Ord
  inst_a}
   : (Backwards inst_f inst_a) -> (Backwards inst_f inst_a) -> bool :=
  fun x y => _/=_ (Ord__Backwards_compare x y) Lt.

Local Definition Ord__Backwards_op_zl__ {inst_f} {inst_a} `{Ord1 inst_f} `{Ord
  inst_a}
   : (Backwards inst_f inst_a) -> (Backwards inst_f inst_a) -> bool :=
  fun x y => _==_ (Ord__Backwards_compare x y) Lt.

Local Definition Ord__Backwards_op_zlze__ {inst_f} {inst_a} `{Ord1 inst_f} `{Ord
  inst_a}
   : (Backwards inst_f inst_a) -> (Backwards inst_f inst_a) -> bool :=
  fun x y => _/=_ (Ord__Backwards_compare x y) Gt.

Local Definition Ord__Backwards_max {inst_f} {inst_a} `{Ord1 inst_f} `{Ord
  inst_a}
...
Local Definition Ord__Backwards_min {inst_f} {inst_a} `{Ord1 inst_f} `{Ord
  inst_a}
   : (Backwards inst_f inst_a) ->
     (Backwards inst_f inst_a) -> (Backwards inst_f inst_a) :=
  fun x y => if Ord__Backwards_op_zlze__ x y : bool then x else y.

Program Instance Ord__Backwards {f} {a} `{Ord1 f} `{Ord a}
   : Ord (Backwards f a) :=
  fun _ k =>
    k {| op_zl____ := Ord__Backwards_op_zl__ ;
         op_zlze____ := Ord__Backwards_op_zlze__ ;
         op_zg____ := Ord__Backwards_op_zg__ ;
         op_zgze____ := Ord__Backwards_op_zgze__ ;
         compare__ := Ord__Backwards_compare ;
         max__ := Ord__Backwards_max ;
         min__ := Ord__Backwards_min |}.
...
Local Definition Functor__Backwards_fmap {inst_f} `{(Functor inst_f)}
   : forall {a} {b}, (a -> b) -> (Backwards inst_f) a -> (Backwards inst_f) b :=
  fun {a} {b} =>
    fun arg_0__ arg_1__ =>
      match arg_0__, arg_1__ with
      | f, Mk_Backwards a => Mk_Backwards (fmap f a)
      end.

Local Definition Functor__Backwards_op_zlzd__ {inst_f} `{(Functor inst_f)}
   : forall {a} {b}, a -> (Backwards inst_f) b -> (Backwards inst_f) a :=
  fun {a} {b} => fun x => Functor__Backwards_fmap (const x).

Program Instance Functor__Backwards {f} `{(Functor f)}
   : Functor (Backwards f) :=
  fun _ k =>
    k {| op_zlzd____ := fun {a} {b} => Functor__Backwards_op_zlzd__ ;
         fmap__ := fun {a} {b} => Functor__Backwards_fmap |}.

Local Definition Applicative__Backwards_op_zlztzg__ {inst_f} `{(Applicative
   inst_f)}
   : forall {a} {b},
  ...
  fun {a} {b} =>
    fun arg_0__ arg_1__ =>
      match arg_0__, arg_1__ with
      | Mk_Backwards f, Mk_Backwards a => Mk_Backwards (a <**> f)
      end.

Local Definition Applicative__Backwards_op_ztzg__ {inst_f} `{(Applicative
   inst_f)}
   : forall {a} {b},
     (Backwards inst_f) a -> (Backwards inst_f) b -> (Backwards inst_f) b :=
  fun {a} {b} =>
    fun x y => Applicative__Backwards_op_zlztzg__ (fmap (const id) x) y.

Local Definition Applicative__Backwards_pure {inst_f} `{(Applicative inst_f)}
   : forall {a}, a -> (Backwards inst_f) a :=
  fun {a} => fun a => Mk_Backwards (pure a).

Program Instance Applicative__Backwards {f} `{(Applicative f)}
   : Applicative (Backwards f) :=
  fun _ k =>
    k {| op_ztzg____ := fun {a} {b} => Applicative__Backwards_op_ztzg__ ;
         op_zlztzg____ := fun {a} {b} => Applicative__Backwards_op_zlztzg__ ;
         pure__ := fun {a} => Applicative__Backwards_pure |}.
...
Local Definition Foldable__Backwards_foldMap {inst_f} `{(Data.Foldable.Foldable
   inst_f)}
   : forall {m} {a}, forall `{Monoid m}, (a -> m) -> (Backwards inst_f) a -> m :=
  fun {m} {a} `{Monoid m} =>
    fun arg_0__ arg_1__ =>
      match arg_0__, arg_1__ with
...
Local Definition Foldable__Backwards_product {inst_f} `{(Data.Foldable.Foldable
  ...
  fun {a} `{GHC.Num.Num a} =>
    Data.Foldable.hash_compose getProduct (Foldable__Backwards_foldMap Mk_Product).

Local Definition Foldable__Backwards_sum {inst_f} `{(Data.Foldable.Foldable
   inst_f)}
   : forall {a}, forall `{GHC.Num.Num a}, (Backwards inst_f) a -> a :=
  fun {a} `{GHC.Num.Num a} =>
    Data.Foldable.hash_compose getSum (Foldable__Backwards_foldMap Mk_Sum).

Local Definition Foldable__Backwards_fold {inst_f} `{(Data.Foldable.Foldable
   inst_f)}
   : forall {m}, forall `{Monoid m}, (Backwards inst_f) m -> m :=
  fun {m} `{Monoid m} => Foldable__Backwards_foldMap id.

Local Definition Foldable__Backwards_elem {inst_f} `{(Data.Foldable.Foldable
   inst_f)}
   : forall {a}, forall `{Eq_ a}, a -> (Backwards inst_f) a -> bool :=
  fun {a} `{Eq_ a} =>
    Coq.Program.Basics.compose (fun arg_69__ =>
                                  let 'p := arg_69__ in
                                  Coq.Program.Basics.compose getAny (Foldable__Backwards_foldMap
  ...
  fun {a} {b} =>
    fun arg_9__ arg_10__ arg_11__ =>
      ...
      | f, z0, xs =>
          ...
          Foldable__Backwards_foldl f' id xs z0
      end.
  ...
  fun {b} {a} =>
    fun arg_24__ arg_25__ arg_26__ =>
      ...
      | f, z0, xs =>
          ...
          Foldable__Backwards_foldr f' id xs z0
      end.
...
Program Instance Foldable__Backwards {f} `{(Data.Foldable.Foldable f)}
  ...
  fun _ k =>
    k {| Data.Foldable.elem__ := fun {a} `{Eq_ a} => Foldable__Backwards_elem ;
         Data.Foldable.fold__ := fun {m} `{Monoid m} => Foldable__Backwards_fold ;
...
Local Definition Traversable__Backwards_sequenceA {inst_f}
  `{(Data.Traversable.Traversable inst_f)}
   : forall {f} {a},
     forall `{Applicative f}, (Backwards inst_f) (f a) -> f ((Backwards inst_f) a) :=
  fun {f} {a} `{Applicative f} =>
    fun arg_0__ =>
      let 'Mk_Backwards t := arg_0__ in
      fmap Mk_Backwards (Data.Traversable.sequenceA t).

Local Definition Traversable__Backwards_sequence {inst_f}
  `{(Data.Traversable.Traversable inst_f)}
   : forall {m} {a},
     forall `{Monad m}, (Backwards inst_f) (m a) -> m ((Backwards inst_f) a) :=
  fun {m} {a} `{Monad m} => Traversable__Backwards_sequenceA.
...
Local Definition Traversable__Backwards_traverse {inst_f}
  ...
  fun {f} {a} {b} `{Applicative f} =>
    fun arg_0__ arg_1__ =>
      match arg_0__, arg_1__ with
      | f, Mk_Backwards t => fmap Mk_Backwards (Data.Traversable.traverse f t)
      end.
...
Local Definition Traversable__Backwards_mapM {inst_f}
  ...
  fun {m} {a} {b} `{Monad m} => Traversable__Backwards_traverse.
...
(* Unbound variables:
     ...
     liftCompare liftEq list negb op_zdzn__ op_zeze__ op_zlztztzg__ op_zsze__ pure
     ...
     Data.Traversable.sequenceA Data.Traversable.traverse GHC.Num.Int GHC.Num.Num
*)
