           ...
           | env, Core.Case e bndr ty alts => cseCase env e bndr ty alts
                      ...
                      := match arg_0__, arg_1__, arg_2__ with
                         ...
                         | _, env, Core.Rec (cons (pair in_id rhs) nil) =>
                             let 'pair env1 (cons out_id nil) := addRecBinders env (cons in_id nil) in
                         ...
                         end with cse_bind arg_0__ arg_1__ arg_2__ arg_3__
                                                                     ...
                                                                     fun arg_5__ =>
                                                                       ...
                                                                       | pair (pair (Core.DataAlt con) args) rhs =>
                                                                           let 'pair env' args' := addBinders alt_env
                                                                                                     args in
                                                                           let con_expr :=
                                                                             CoreUtils.mkAltExpr (Core.DataAlt con)
                                                                             args' arg_tys in
                                                                           let new_env :=
                                                                             extendCSEnv env' con_expr con_target in
                                                                           if negb (Data.Foldable.null args) : bool
                                                                           then pair (pair (Core.DataAlt con) args')
                                                                                     (tryForCSE new_env rhs) else
                                                                           j_8__
                                                                       | _ => j_8__
                                                                       end in
                                                                   let ty' := tt in
                                                                   Core.Case scrut1 bndr3 ty' (combineAlts alt_env
                                                                                                           (GHC.Base.map
                                                                                                            cse_alt
                                                                                                            alts))
  for cseBind.
