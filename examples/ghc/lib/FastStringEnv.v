...
Definition alterFsEnv {a} : (option a -> option a) -> FastStringEnv
                            a -> FastString.FastString -> FastStringEnv a :=
  UniqFM.alterUFM.

Definition delFromFsEnv {a} : FastStringEnv
                              a -> FastString.FastString -> FastStringEnv a :=
  fun x y => UniqFM.delFromUFM x y.

Definition delListFromFsEnv {a} : FastStringEnv a -> list
                                  FastString.FastString -> FastStringEnv a :=
...
Definition extendFsEnv {a} : FastStringEnv
                             a -> FastString.FastString -> a -> FastStringEnv a :=
  fun x y z => UniqFM.addToUFM x y z.

Definition extendFsEnvList {a} : FastStringEnv a -> list (FastString.FastString
                                                         * a)%type -> FastStringEnv a :=
  fun x l => UniqFM.addListToUFM x l.

Definition extendFsEnvList_C {a} : (a -> a -> a) -> FastStringEnv a -> list
                                   (FastString.FastString * a)%type -> FastStringEnv a :=
  fun x y z => UniqFM.addListToUFM_C x y z.

Definition extendFsEnv_Acc {a} {b} : (a -> b -> b) -> (a -> b) -> FastStringEnv
                                     b -> FastString.FastString -> a -> FastStringEnv b :=
  fun x y z a b => UniqFM.addToUFM_Acc x y z a b.

Definition extendFsEnv_C {a} : (a -> a -> a) -> FastStringEnv
                               a -> FastString.FastString -> a -> FastStringEnv a :=
  fun f x y z => UniqFM.addToUFM_C f x y z.

Definition filterFsEnv {elt} : (elt -> bool) -> FastStringEnv
                               elt -> FastStringEnv elt :=
...
Definition fsEnvUniqueElts {a} : FastStringEnv a -> list (Unique.Unique *
                                                         a)%type :=
  fun x => UniqFM.ufmToList x.

Definition lookupFsEnv {a} : FastStringEnv a -> FastString.FastString -> option
                             a :=
  fun x y => UniqFM.lookupUFM x y.

Definition mapFsEnv {elt1} {elt2} : (elt1 -> elt2) -> FastStringEnv
                                    elt1 -> FastStringEnv elt2 :=
  fun f x => UniqFM.mapUFM f x.

Definition mkFsEnv {a} : list (FastString.FastString * a)%type -> FastStringEnv
                         a :=
  fun l => UniqFM.listToUFM l.

Definition plusFsEnv {a} : FastStringEnv a -> FastStringEnv a -> FastStringEnv
                           a :=
  fun x y => UniqFM.plusUFM x y.

Definition plusFsEnv_C {a} : (a -> a -> a) -> FastStringEnv a -> FastStringEnv
                             a -> FastStringEnv a :=
