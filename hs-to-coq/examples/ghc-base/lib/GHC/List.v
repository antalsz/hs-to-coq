...
Fixpoint take {a:Set} (n:Z) (xs:list a) : list a :=
  if (n <=? 0)%Z then nil
  else match xs with
       | nil => nil
...
Fixpoint drop {a:Set} (n:Z) (xs:list a) : list a :=
  if (n <=? 0)%Z then xs
  else match xs with
       | nil => nil
...
Fixpoint scanr {a b:Set} (f : a -> b -> b) (q0 : b)
        (xs: list a) :=
 match xs with
 | nil => (cons q0 nil)
...
Definition splitAt {a : Set}(n:Z)(xs:list a) :=
  (take n xs, drop n xs).

Definition replicate {a: Set}(n:Z): a -> list a.
destruct (0 <=? n)%Z eqn:E; [|exact (fun _ => nil)].
apply Zle_bool_imp_le in E.
apply natlike_rec with (P := fun _ => a -> list a)(x := n).
- exact (fun _ => nil).
