...
Local Definition Eq1__ExceptT_liftEq {inst_e} {inst_m} `{Eq_ inst_e} `{Eq1
  inst_m}
   : forall {a} {b},
     (a -> b -> bool) ->
...
Program Instance Eq1__ExceptT {e} {m} `{Eq_ e} `{Eq1 m} : Eq1 (ExceptT e m) :=
  fun _ k => k {| liftEq__ := fun {a} {b} => Eq1__ExceptT_liftEq |}.

Local Definition Ord1__ExceptT_liftCompare {inst_e} {inst_m} `{Ord inst_e}
  `{Ord1 inst_m}
   : forall {a} {b},
     (a -> b -> comparison) ->
...
Program Instance Ord1__ExceptT {e} {m} `{Ord e} `{Ord1 m}
   : Ord1 (ExceptT e m) :=
...
Local Definition Eq___ExceptT_op_zeze__ {inst_e} {inst_m} {inst_a} `{Eq_ inst_e}
  `{Eq1 inst_m} `{Eq_ inst_a}
   : (ExceptT inst_e inst_m inst_a) -> (ExceptT inst_e inst_m inst_a) -> bool :=
  eq1.

Local Definition Eq___ExceptT_op_zsze__ {inst_e} {inst_m} {inst_a} `{Eq_ inst_e}
  `{Eq1 inst_m} `{Eq_ inst_a}
   : (ExceptT inst_e inst_m inst_a) -> (ExceptT inst_e inst_m inst_a) -> bool :=
  fun x y => negb (Eq___ExceptT_op_zeze__ x y).

Program Instance Eq___ExceptT {e} {m} {a} `{Eq_ e} `{Eq1 m} `{Eq_ a}
   : Eq_ (ExceptT e m a) :=
  fun _ k =>
    k {| op_zeze____ := Eq___ExceptT_op_zeze__ ;
         op_zsze____ := Eq___ExceptT_op_zsze__ |}.

Local Definition Ord__ExceptT_compare {inst_e} {inst_m} {inst_a} `{Ord inst_e}
  `{Ord1 inst_m} `{Ord inst_a}
   : (ExceptT inst_e inst_m inst_a) ->
     (ExceptT inst_e inst_m inst_a) -> comparison :=
  compare1.

Local Definition Ord__ExceptT_op_zg__ {inst_e} {inst_m} {inst_a} `{Ord inst_e}
  `{Ord1 inst_m} `{Ord inst_a}
   : (ExceptT inst_e inst_m inst_a) -> (ExceptT inst_e inst_m inst_a) -> bool :=
  fun x y => _==_ (Ord__ExceptT_compare x y) Gt.

Local Definition Ord__ExceptT_op_zgze__ {inst_e} {inst_m} {inst_a} `{Ord inst_e}
  `{Ord1 inst_m} `{Ord inst_a}
   : (ExceptT inst_e inst_m inst_a) -> (ExceptT inst_e inst_m inst_a) -> bool :=
  fun x y => _/=_ (Ord__ExceptT_compare x y) Lt.

Local Definition Ord__ExceptT_op_zl__ {inst_e} {inst_m} {inst_a} `{Ord inst_e}
  `{Ord1 inst_m} `{Ord inst_a}
   : (ExceptT inst_e inst_m inst_a) -> (ExceptT inst_e inst_m inst_a) -> bool :=
  fun x y => _==_ (Ord__ExceptT_compare x y) Lt.

Local Definition Ord__ExceptT_op_zlze__ {inst_e} {inst_m} {inst_a} `{Ord inst_e}
  `{Ord1 inst_m} `{Ord inst_a}
   : (ExceptT inst_e inst_m inst_a) -> (ExceptT inst_e inst_m inst_a) -> bool :=
  fun x y => _/=_ (Ord__ExceptT_compare x y) Gt.

Local Definition Ord__ExceptT_max {inst_e} {inst_m} {inst_a} `{Ord inst_e}
  `{Ord1 inst_m} `{Ord inst_a}
   : (ExceptT inst_e inst_m inst_a) ->
     (ExceptT inst_e inst_m inst_a) -> (ExceptT inst_e inst_m inst_a) :=
  fun x y => if Ord__ExceptT_op_zlze__ x y : bool then y else x.

Local Definition Ord__ExceptT_min {inst_e} {inst_m} {inst_a} `{Ord inst_e}
  `{Ord1 inst_m} `{Ord inst_a}
   : (ExceptT inst_e inst_m inst_a) ->
     (ExceptT inst_e inst_m inst_a) -> (ExceptT inst_e inst_m inst_a) :=
  fun x y => if Ord__ExceptT_op_zlze__ x y : bool then x else y.

Program Instance Ord__ExceptT {e} {m} {a} `{Ord e} `{Ord1 m} `{Ord a}
   : Ord (ExceptT e m a) :=
  fun _ k =>
    k {| op_zl____ := Ord__ExceptT_op_zl__ ;
         op_zlze____ := Ord__ExceptT_op_zlze__ ;
         op_zg____ := Ord__ExceptT_op_zg__ ;
         op_zgze____ := Ord__ExceptT_op_zgze__ ;
         compare__ := Ord__ExceptT_compare ;
         max__ := Ord__ExceptT_max ;
         min__ := Ord__ExceptT_min |}.
...
Local Definition Foldable__ExceptT_foldMap {inst_f} {inst_e}
  `{(Data.Foldable.Foldable inst_f)}
   : forall {m} {a},
     forall `{Monoid m}, (a -> m) -> (ExceptT inst_e inst_f) a -> m :=
  fun {m} {a} `{Monoid m} =>
    fun arg_0__ arg_1__ =>
      match arg_0__, arg_1__ with
      | f, Mk_ExceptT a =>
          Data.Foldable.foldMap (Data.Either.either (const mempty) f) a
      end.
  ...
  fun {a} {b} =>
    fun arg_9__ arg_10__ arg_11__ =>
      ...
      | f, z0, xs =>
          ...
          Foldable__ExceptT_foldl f' id xs z0
      end.
  ...
  fun {b} {a} =>
    fun arg_24__ arg_25__ arg_26__ =>
      ...
      | f, z0, xs =>
          ...
          Foldable__ExceptT_foldr f' id xs z0
      end.
...
Local Definition Foldable__ExceptT_fold {inst_f} {inst_e}
  `{(Data.Foldable.Foldable inst_f)}
   : forall {m}, forall `{Monoid m}, (ExceptT inst_e inst_f) m -> m :=
  fun {m} `{Monoid m} => Foldable__ExceptT_foldMap id.

Local Definition Foldable__ExceptT_elem {inst_f} {inst_e}
  `{(Data.Foldable.Foldable inst_f)}
   : forall {a}, forall `{Eq_ a}, a -> (ExceptT inst_e inst_f) a -> bool :=
  fun {a} `{Eq_ a} =>
...
Program Instance Foldable__ExceptT {f} {e} `{(Data.Foldable.Foldable f)}
  ...
  fun _ k =>
    k {| Data.Foldable.elem__ := fun {a} `{Eq_ a} => Foldable__ExceptT_elem ;
         Data.Foldable.fold__ := fun {m} `{Monoid m} => Foldable__ExceptT_fold ;
  ...
  fun {f} {a} {b} `{Applicative f} =>
    fun arg_0__ arg_1__ =>
      ...
      | f, Mk_ExceptT a =>
          ...
          Data.Traversable.traverse (Data.Either.either (pure ∘ Data.Either.Left) (fmap
                                                                                   Data.Either.Right ∘
                                                                                   f)) a
      end.
...
Local Definition Traversable__ExceptT_sequenceA {inst_f} {inst_e}
  ...
  fun {f} {a} `{Applicative f} => Traversable__ExceptT_traverse id.
...
Local Definition Traversable__ExceptT_sequence {inst_f} {inst_e}
  ...
  fun {m} {a} `{Monad m} => Traversable__ExceptT_sequenceA.
...
Local Definition Traversable__ExceptT_mapM {inst_f} {inst_e}
  ...
  fun {m} {a} {b} `{Monad m} => Traversable__ExceptT_traverse.

Local Definition Applicative__ExceptT_op_zlztzg__ {inst_m} {inst_e} `{Functor
  inst_m} `{Monad inst_m}
   : forall {a} {b},
  ...
  fun {a} {b} =>
    fun arg_0__ arg_1__ =>
      ...
      | Mk_ExceptT f, Mk_ExceptT v =>
          Mk_ExceptT (_>>=_ f (fun mf =>
                               match mf with
                               | Data.Either.Left e => return_ (Data.Either.Left e)
                               | Data.Either.Right k =>
                                   v >>=
                                   (fun mv =>
                                      match mv with
                                      | Data.Either.Left e => return_ (Data.Either.Left e)
                                      | Data.Either.Right x => return_ (Data.Either.Right (k x))
                                      end)
                               end))
      end.
...
Local Definition MonadTrans__ExceptT_lift {inst_e}
   : forall {m} {a} `{Monad m}, m a -> (ExceptT inst_e) m a :=
  fun {m} {a} `{Monad m} => Mk_ExceptT ∘ liftM Data.Either.Right.
...
Definition withExceptT {m} {e} {e'} {a} `{(Functor m)}
   : (e -> e') -> ExceptT e m a -> ExceptT e' m a :=
  fun f =>
    mapExceptT (fmap (Data.Either.either (Data.Either.Left ∘ f) Data.Either.Right)).
...
Definition mapExcept {e} {a} {e'} {b}
  ...
  fun f => mapExceptT (Mk_Identity ∘ (f ∘ runIdentity)).

Definition liftPass {m} {w} {e} {a} `{(Monad m)}
   : Control.Monad.Signatures.Pass w m (Data.Either.Either e a) ->
     Control.Monad.Signatures.Pass w (ExceptT e m) a :=
  fun pass =>
    mapExceptT (fun m =>
                  pass (_>>=_ m (fun a =>
                                 return_ (match a with
                                          | Data.Either.Left l => pair (Data.Either.Left l) id
                                          | Data.Either.Right (pair r f) => pair (Data.Either.Right r) f
                                          end)))).

Definition liftListen {m} {w} {e} {a} `{(Monad m)}
  ...
  fun listen =>
    mapExceptT (fun m =>
                  let cont_0__ arg_1__ :=
                    ...
                    return_ (fmap (fun r => pair r w) a) in
                  listen m >>= cont_0__).
...
Definition catchE {m} {e} {a} {e'} `{(Monad m)}
   : ExceptT e m a -> (e -> ExceptT e' m a) -> ExceptT e' m a :=
  fun m h =>
    Mk_ExceptT (_>>=_ (runExceptT m) (fun a =>
                         match a with
                         | Data.Either.Left l => runExceptT (h l)
                         | Data.Either.Right r => return_ (Data.Either.Right r)
                         end)).

Local Definition Functor__ExceptT_fmap {inst_m} {inst_e} `{(Functor inst_m)}
   : forall {a} {b},
     (a -> b) -> (ExceptT inst_e inst_m) a -> (ExceptT inst_e inst_m) b :=
  fun {a} {b} => fun f => Mk_ExceptT ∘ (fmap (fmap f) ∘ runExceptT).

Local Definition Functor__ExceptT_op_zlzd__ {inst_m} {inst_e} `{(Functor
   inst_m)}
   : forall {a} {b},
     a -> (ExceptT inst_e inst_m) b -> (ExceptT inst_e inst_m) a :=
  fun {a} {b} => fun x => Functor__ExceptT_fmap (const x).

Program Instance Functor__ExceptT {m} {e} `{(Functor m)}
   : Functor (ExceptT e m) :=
  fun _ k =>
    k {| op_zlzd____ := fun {a} {b} => Functor__ExceptT_op_zlzd__ ;
         fmap__ := fun {a} {b} => Functor__ExceptT_fmap |}.

Local Definition Applicative__ExceptT_pure {inst_m} {inst_e} `{Functor inst_m}
  `{Monad inst_m}
   : forall {a}, a -> (ExceptT inst_e inst_m) a :=
  fun {a} => fun a => Mk_ExceptT (return_ (Data.Either.Right a)).

Definition Applicative__ExceptT_op_ztzg__ {inst_m} {inst_s} `{_ : Monad inst_m}
   : forall {a} {b},
...
Program Instance Applicative__ExceptT {m} {e} `{Functor m} `{Monad m}
   : Applicative (ExceptT e m) :=
  fun _ k =>
    k {| op_ztzg____ := fun {a} {b} => Applicative__ExceptT_op_ztzg__ ;
         op_zlztzg____ := fun {a} {b} => Applicative__ExceptT_op_zlztzg__ ;
         pure__ := fun {a} => Applicative__ExceptT_pure |}.

Local Definition Monad__ExceptT_op_zgzg__ {inst_m} {inst_e} `{(Monad inst_m)}
   : forall {a} {b},
     (ExceptT inst_e inst_m) a ->
     (ExceptT inst_e inst_m) b -> (ExceptT inst_e inst_m) b :=
  fun {a} {b} => _*>_.

Definition Monad__ExceptT_op_zgzgze__ {inst_m} {inst_e} `{_ : Monad inst_m}
   : forall {a} {b},
     ExceptT inst_e inst_m a ->
...
Local Definition Monad__ExceptT_return_ {inst_m} {inst_e} `{(Monad inst_m)}
   : forall {a}, a -> (ExceptT inst_e inst_m) a :=
  fun {a} => pure.

Program Instance Monad__ExceptT {m} {e} `{(Monad m)} : Monad (ExceptT e m) :=
  fun _ k =>
    k {| op_zgzg____ := fun {a} {b} => Monad__ExceptT_op_zgzg__ ;
         op_zgzgze____ := fun {a} {b} => Monad__ExceptT_op_zgzgze__ ;
         return___ := fun {a} => Monad__ExceptT_return_ |}.
...
Definition throwE {m} {e} {a} `{(Monad m)} : e -> ExceptT e m a :=
  Mk_ExceptT ∘ (return_ ∘ Data.Either.Left).

(* Unbound variables:
     Applicative Eq1 Eq_ Functor Gt Identity Lt Mk_Any Mk_Dual Mk_Endo Mk_Identity
     Mk_Product Mk_Sum Monad Monoid Ord Ord1 appEndo bool build compare1 comparison
     const eq1 false flip fmap getAny getDual getProduct getSum id liftCompare liftEq
     liftM list mempty negb op_z2218U__ op_zdzn__ op_zeze__ op_zgzgze__ op_zsze__
     op_ztzg__ pair pure return_ runIdentity true Control.Monad.Signatures.CallCC
