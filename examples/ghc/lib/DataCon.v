...
Inductive DataConRep : Type := NoDataConRep : DataConRep
                            |  DCR : Var.Id -> unit -> list unit -> list StrictnessMark -> list
                                     HsImplBang -> DataConRep.

Inductive DataCon : Type := MkData
                           : Name.Name -> Unique.Unique -> BasicTypes.ConTag -> bool -> list
                             Var.TyVar -> list unit -> list Var.TyVar -> list unit -> list
                             EqSpec -> unit -> unit -> list unit -> unit -> list HsSrcBang -> list
                             FieldLabel.FieldLabel -> Var.Id -> DataConRep -> BasicTypes.Arity -> BasicTypes.Arity -> unit -> unit -> bool -> unit -> DataCon.
...
Instance Eq___DataCon : GHC.Base.Eq_ DataCon := {}.
Proof.
Admitted.

Instance Ord__DataCon : GHC.Base.Ord DataCon := {}.
Proof.
Admitted.

Instance Eq___SrcUnpackedness : GHC.Base.Eq_ SrcUnpackedness := {}.
Proof.
Admitted.

Instance Eq___SrcStrictness : GHC.Base.Eq_ SrcStrictness := {}.
Proof.
Admitted.
...
(* Unbound variables:
     bool error list option unit BasicTypes.Arity BasicTypes.ConTag
     BasicTypes.SourceText FieldLabel.FieldLabel GHC.Base.Eq_ GHC.Base.Ord Name.Name
     Unique.Unique Var.Id Var.TyVar
*)
