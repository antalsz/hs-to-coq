  ...
  fun arg_0__ =>
    ...
    | Decode.Auipc rd oimm20 =>
        Monads.Bind Program.getPC (fun pc =>
                       Program.setRegister rd (Utility.fromImm oimm20 + pc))
    | Decode.Jal rd jimm20 =>
        Monads.Bind Program.getPC (fun pc =>
                       let newPC := pc + Utility.fromImm jimm20 in
                       if (GHC.Real.mod_ newPC 4 /= 0) : bool
                       then Program.raiseException 0 0
                       else (Monads.Bind (Program.setRegister rd (pc + 4)) (fun _ =>
                                            Program.setPC newPC)))
    | Decode.Jalr rd rs1 oimm12 =>
        Monads.Bind (Program.getRegister rs1) (fun x =>
                       Monads.Bind Program.getPC (fun pc =>
                                      let newPC :=
                                        Coq.ZArith.BinInt.Z.land (x + Utility.fromImm oimm12) (Coq.ZArith.BinInt.Z.lnot
                                                                  1) in
                                      if (GHC.Real.mod_ newPC 4 /= 0) : bool
                                      then Program.raiseException 0 0
                                      else (Monads.Bind (Program.setRegister rd (pc + 4)) (fun _ =>
                       ...
                       Monads.Bind (Program.getRegister rs2) (fun y =>
                                      Monads.Bind Program.getPC (fun pc =>
                                                     Utility.when (x /= y) (let addr :=
                                                                              (pc + Utility.fromImm sbimm12) in
                                                                            if (GHC.Real.mod_ addr 4 /= 0) : bool
                                                                            then Program.raiseException 0 0
    ...
    | Decode.Blt rs1 rs2 sbimm12 =>
        Monads.Bind (Program.getRegister rs1) (fun x =>
                       Monads.Bind (Program.getRegister rs2) (fun y =>
                                      Monads.Bind Program.getPC (fun pc =>
                                                     Utility.when (x < y) (let addr := (pc + Utility.fromImm sbimm12) in
                                                                           if (GHC.Real.mod_ addr 4 /= 0) : bool
                                                                           then Program.raiseException 0 0
                                                                           else Program.setPC addr))))
    ...
    | Decode.Sb rs1 rs2 simm12 =>
        Monads.Bind (Program.getRegister rs1) (fun a =>
                       Monads.Bind (Program.translate Program.Store 1 (a + Utility.fromImm simm12))
                                   (fun addr =>
                                      Monads.Bind (Program.getRegister rs2) (fun x =>
                                                     Program.storeByte addr (Utility.regToInt8 x))))
    | Decode.Sh rs1 rs2 simm12 =>
        Monads.Bind (Program.getRegister rs1) (fun a =>
                       Monads.Bind (Program.translate Program.Store 2 (a + Utility.fromImm simm12))
                                   (fun addr =>
                                      Monads.Bind (Program.getRegister rs2) (fun x =>
                                                     Program.storeHalf addr (Utility.regToInt16 x))))
    | Decode.Sw rs1 rs2 simm12 =>
        Monads.Bind (Program.getRegister rs1) (fun a =>
                       Monads.Bind (Program.translate Program.Store 4 (a + Utility.fromImm simm12))
                                   (fun addr =>
                                      Monads.Bind (Program.getRegister rs2) (fun x =>
                                                     Program.storeWord addr (Utility.regToInt32 x))))
    | Decode.Addi rd rs1 imm12 =>
        Monads.Bind (Program.getRegister rs1) (fun x =>
                       Program.setRegister rd (x + Utility.fromImm imm12))
    ...
    | Decode.Add rd rs1 rs2 =>
        Monads.Bind (Program.getRegister rs1) (fun x =>
                       Monads.Bind (Program.getRegister rs2) (fun y => Program.setRegister rd (x + y)))
...
(* Unbound variables:
     bool negb op_zgze__ op_zl__ op_zm__ op_zp__ op_zsze__ tt unit
     Coq.ZArith.BinInt.Z.land Coq.ZArith.BinInt.Z.lnot Coq.ZArith.BinInt.Z.lor
     Coq.ZArith.BinInt.Z.lxor Decode.Add Decode.Addi Decode.And Decode.Andi
     Decode.Auipc Decode.Beq Decode.Bge Decode.Bgeu Decode.Blt Decode.Bltu Decode.Bne
     ...
     Decode.Xori GHC.Real.mod_ Monads.Bind Monads.Return Program.Load
     ...
     Utility.regToInt32 Utility.regToInt8 Utility.regToShamt Utility.signed_eqb
