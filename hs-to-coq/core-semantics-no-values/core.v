...
Inductive TypeOrdering : Type := Mk_TLT : TypeOrdering
                              |  Mk_TEQ : TypeOrdering
                              |  Mk_TEQX : TypeOrdering
                              |  Mk_TGT : TypeOrdering.
...
Inductive ModuleEnv elt : Type := Mk_ModuleEnv : (((Map NDModule
                                                        elt)) -> (ModuleEnv elt)).

Definition ModuleSet := ((Set_ NDModule)%type).
...
Definition DTyVarEnv elt := ((DVarEnv elt)%type).

Definition DNameEnv a := ((UniqDFM a)%type).

Definition DIdEnv elt := ((DVarEnv elt)%type).
...
with ClassATItem : Type := Mk_ATI : (TyCon -> (((option (Type_ *
                                                        SrcSpan))) -> ClassATItem))
with TyCon : Type := Mk_FunTyCon : (Unique -> (Name -> ((list
                                   TyBinder) -> (Kind -> (Kind -> (Arity -> (TyConRepName -> TyCon)))))))
                  |  Mk_AlgTyCon : (Unique -> (Name -> ((list
                                   TyBinder) -> (Kind -> (Kind -> (Arity -> ((list TyVar) -> ((list
                                   Role) -> ((option CType) -> (bool -> ((list
                                   PredType) -> (AlgTyConRhs -> (FieldLabelEnv -> (RecFlag -> (AlgTyConFlav -> TyCon)))))))))))))))
                  |  Mk_SynonymTyCon : (Unique -> (Name -> ((list
                                       TyBinder) -> (Kind -> (Kind -> (Arity -> ((list TyVar) -> ((list
                                       Role) -> (Type_ -> TyCon)))))))))
                  |  Mk_FamilyTyCon : (Unique -> (Name -> ((list
                                      TyBinder) -> (Kind -> (Kind -> (Arity -> ((list TyVar) -> ((option
                                      Name) -> (FamTyConFlav -> ((option Class) -> (Injectivity -> TyCon)))))))))))
                  |  Mk_PrimTyCon : (Unique -> (Name -> ((list
                                    TyBinder) -> (Kind -> (Kind -> (Arity -> ((list Role) -> (bool -> ((option
                                    TyConRepName) -> TyCon)))))))))
                  |  Mk_PromotedDataCon : (Unique -> (Name -> (Arity -> ((list
                                          TyBinder) -> (Kind -> (Kind -> ((list
                                          Role) -> (DataCon -> (TyConRepName -> (RuntimeRepInfo -> TyCon))))))))))
                  |  Mk_TcTyCon : (Unique -> (Name -> (bool -> ((list TyVar) -> ((list
                                  TyBinder) -> (Kind -> (Kind -> (Arity -> ((list TyVar) -> TyCon)))))))))
...
with DataCon : Type := Mk_MkData
                      : (Name -> (Unique -> (ConTag -> (bool -> ((list TyVar) -> ((list
                        TyBinder) -> ((list TyVar) -> ((list TyBinder) -> ((list
                        EqSpec) -> (ThetaType -> (ThetaType -> ((list Type_) -> (Type_ -> ((list
                        HsSrcBang) -> ((list
...
with PatSyn : Type := Mk_MkPatSyn : (Name -> (Unique -> ((list
                                    Type_) -> (Arity -> (bool -> ((list FieldLabel) -> ((list TyVar) -> ((list
                                    TyBinder) -> (ThetaType -> ((list TyVar) -> ((list
                                    TyBinder) -> (ThetaType -> (Type_ -> ((Id * bool) -> ((option (Id *
                                                                                                  bool)) -> PatSyn)))))))))))))))
...
Inductive NormaliseStepResult ev : Type := Mk_NS_Done : (NormaliseStepResult ev)
                                        |  Mk_NS_Abort : (NormaliseStepResult ev)
                                        |  Mk_NS_Step : (RecTcChecker -> (Type_ -> (ev -> (NormaliseStepResult ev)))).

Definition NormaliseStepper ev := ((RecTcChecker -> (TyCon -> ((list
                                  Type_) -> (NormaliseStepResult ev))))%type).
...
Inductive TcDepVars : Type := Mk_DV : (DTyCoVarSet -> (DTyVarSet -> TcDepVars)).
...
Inductive Expr : (forall b, Type) := Mk_Var : (forall {b}, (Id -> (Expr b)))
                                  ...
                                  |  Mk_App : (forall {b}, (((Expr b)) -> (((Arg b)) -> (Expr b))))
...
Inductive AnnExpr' bndr annot : Type := Mk_AnnVar : (Id -> (AnnExpr' bndr
                                     ...
                                     |  Mk_AnnApp : (((AnnExpr bndr annot)) -> (((AnnExpr bndr annot)) -> (AnnExpr'
                                                    bndr annot)))
                                     |  Mk_AnnCase : (((AnnExpr bndr annot)) -> (bndr -> (Type_ -> ((list (AnnAlt
                                                                                                          bndr
                                                                                                          annot)) -> (AnnExpr'
                                                     bndr annot)))))
                                     |  Mk_AnnLet : (((AnnBind bndr annot)) -> (((AnnExpr bndr annot)) -> (AnnExpr'
                                                    bndr annot)))
                                     |  Mk_AnnCast : (((AnnExpr bndr annot)) -> ((annot * Coercion) -> (AnnExpr' bndr
                                                                                                                 annot)))
...
Inductive UserTypeCtxt : Type := Mk_FunSigCtxt
                              ...
                              |  Mk_PatSynBuilderCtxt : (Name -> UserTypeCtxt)
...
Arguments Mk_NS_Done {_}.

Arguments Mk_NS_Abort {_}.
...
Definition unNDModule (__arg_17__ : NDModule) := (match __arg_17__ with
                                                   | (Mk_NDModule unNDModule) => unNDModule
                                                 end).

Definition ml_hi_file (__arg_18__ : ModLocation) := (match __arg_18__ with
                                                      | (Mk_ModLocation _ ml_hi_file _) => ml_hi_file
                                                    end).

Definition ml_hs_file (__arg_19__ : ModLocation) := (match __arg_19__ with
                                                      | (Mk_ModLocation ml_hs_file _ _) => ml_hs_file
                                                    end).

Definition ml_obj_file (__arg_20__ : ModLocation) := (match __arg_20__ with
                                                       | (Mk_ModLocation _ _ ml_obj_file) => ml_obj_file
                                                     end).

Definition sd {s} {u} (__arg_21__ : (JointDmd s u)) := (match __arg_21__ with
                                                         | (Mk_JD sd _) => sd
                                                       end).

Definition ud {s} {u} (__arg_22__ : (JointDmd s u)) := (match __arg_22__ with
                                                         | (Mk_JD _ ud) => ud
                                                       end).

Definition fl_text (__arg_23__ : FractionalLit) := (match __arg_23__ with
                                                     | (Mk_FL fl_text _) => fl_text
                                                   end).

Definition fl_value (__arg_24__ : FractionalLit) := (match __arg_24__ with
                                                      | (Mk_FL _ fl_value) => fl_value
                                                    end).

Definition flIsOverloaded {a} (__arg_25__ : (FieldLbl a)) :=
  (match __arg_25__ with
    | (Mk_FieldLabel _ flIsOverloaded _) => flIsOverloaded
  end).

Definition flLabel {a} (__arg_26__ : (FieldLbl a)) := (match __arg_26__ with
                                                        | (Mk_FieldLabel flLabel _ _) => flLabel
                                                      end).

Definition flSelector {a} (__arg_27__ : (FieldLbl a)) := (match __arg_27__ with
                                                           | (Mk_FieldLabel _ _ flSelector) => flSelector
                                                         end).

Definition fromEP {a} (__arg_28__ : (EP a)) := (match __arg_28__ with
                                                 | (Mk_EP fromEP _) => fromEP
                                               end).

Definition toEP {a} (__arg_29__ : (EP a)) := (match __arg_29__ with
                                               | (Mk_EP _ toEP) => toEP
                                             end).

Definition cc_is_caf (__arg_30__ : CostCentre) := (match __arg_30__ with
...
Definition cc_key (__arg_31__ : CostCentre) := (match __arg_31__ with
...
Definition cc_loc (__arg_32__ : CostCentre) := (match __arg_32__ with
                                                 | (Mk_NormalCC _ _ _ cc_loc _) => cc_loc
                                                 | (Mk_AllCafsCC _ cc_loc) => cc_loc
                                               end).

Definition cc_mod (__arg_33__ : CostCentre) := (match __arg_33__ with
                                                 | (Mk_NormalCC _ _ cc_mod _ _) => cc_mod
                                                 | (Mk_AllCafsCC cc_mod _) => cc_mod
                                               end).

Definition cc_name (__arg_34__ : CostCentre) := (match __arg_34__ with
...
Definition breakpointFVs {id} (__arg_35__ : (Tickish id)) :=
...
Definition breakpointId {id} (__arg_36__ : (Tickish id)) :=
  (match __arg_36__ with
    | (Mk_ProfNote _ _ _) => (error
...
Definition profNoteCC {id} (__arg_37__ : (Tickish id)) := (match __arg_37__ with
...
Definition profNoteCount {id} (__arg_38__ : (Tickish id)) :=
...
Definition profNoteScope {id} (__arg_39__ : (Tickish id)) :=
...
Definition sourceName {id} (__arg_40__ : (Tickish id)) := (match __arg_40__ with
                                                            | (Mk_ProfNote _ _ _) => (error
...
Definition sourceSpan {id} (__arg_41__ : (Tickish id)) := (match __arg_41__ with
                                                            | (Mk_ProfNote _ _ _) => (error
...
Definition tickId {id} (__arg_42__ : (Tickish id)) := (match __arg_42__ with
                                                        | (Mk_ProfNote _ _ _) => (error
...
Definition tickModule {id} (__arg_43__ : (Tickish id)) := (match __arg_43__ with
                                                            | (Mk_ProfNote _ _ _) => (error
...
Definition ug_args (__arg_44__ : UnfoldingGuidance) := (match __arg_44__ with
...
Definition ug_arity (__arg_45__ : UnfoldingGuidance) := (match __arg_45__ with
...
Definition ug_boring_ok (__arg_46__ : UnfoldingGuidance) :=
...
Definition ug_res (__arg_47__ : UnfoldingGuidance) := (match __arg_47__ with
                                                        | (Mk_UnfWhen _ _ _) => (error
...
Definition ug_size (__arg_48__ : UnfoldingGuidance) := (match __arg_48__ with
                                                         | (Mk_UnfWhen _ _ _) => (error
...
Definition ug_unsat_ok (__arg_49__ : UnfoldingGuidance) :=
  (match __arg_49__ with
...
Definition algTcParent (__arg_62__ : TyCon) := (match __arg_62__ with
                                                 | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                                                  &"Partial record selector: field `algTcParent' has no match in constructor `Mk_FunTyCon' of type `TyCon'")
                                                 | (Mk_AlgTyCon _ _ _ _ _ _ _ _ _ _ _ _ _ _ algTcParent) => algTcParent
                                                 | (Mk_SynonymTyCon _ _ _ _ _ _ _ _ _) => (error
...
Definition algTcRec (__arg_63__ : TyCon) := (match __arg_63__ with
                                              | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                                               &"Partial record selector: field `algTcRec' has no match in constructor `Mk_FunTyCon' of type `TyCon'")
                                              | (Mk_AlgTyCon _ _ _ _ _ _ _ _ _ _ _ _ _ algTcRec _) => algTcRec
                                              | (Mk_SynonymTyCon _ _ _ _ _ _ _ _ _) => (error
...
Definition algTcRhs (__arg_64__ : TyCon) := (match __arg_64__ with
                                              | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                                               &"Partial record selector: field `algTcRhs' has no match in constructor `Mk_FunTyCon' of type `TyCon'")
                                              | (Mk_AlgTyCon _ _ _ _ _ _ _ _ _ _ _ algTcRhs _ _ _) => algTcRhs
                                              | (Mk_SynonymTyCon _ _ _ _ _ _ _ _ _) => (error
...
Definition algTcStupidTheta (__arg_65__ : TyCon) := (match __arg_65__ with
                                                      | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                                                       &"Partial record selector: field `algTcStupidTheta' has no match in constructor `Mk_FunTyCon' of type `TyCon'")
                                                      | (Mk_AlgTyCon _ _ _ _ _ _ _ _ _ _ algTcStupidTheta _ _ _ _) =>
                                                        algTcStupidTheta
                                                      | (Mk_SynonymTyCon _ _ _ _ _ _ _ _ _) => (error
...
Definition dataCon (__arg_66__ : TyCon) := (match __arg_66__ with
                                             | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                                              &"Partial record selector: field `dataCon' has no match in constructor `Mk_FunTyCon' of type `TyCon'")
                                             | (Mk_AlgTyCon _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) => (error
                                             ...
                                             | (Mk_PromotedDataCon _ _ _ _ _ _ _ dataCon _ _) => dataCon
                                             | (Mk_TcTyCon _ _ _ _ _ _ _ _ _) => (error
                                                                                 &"Partial record selector: field `dataCon' has no match in constructor `Mk_TcTyCon' of type `TyCon'")
                                           end).

Definition famTcFlav (__arg_67__ : TyCon) := (match __arg_67__ with
                                               | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                               ...
                                               | (Mk_FamilyTyCon _ _ _ _ _ _ _ _ famTcFlav _ _) => famTcFlav
                                               | (Mk_PrimTyCon _ _ _ _ _ _ _ _ _) => (error
...
Definition famTcInj (__arg_68__ : TyCon) := (match __arg_68__ with
                                              | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                              ...
                                              | (Mk_FamilyTyCon _ _ _ _ _ _ _ _ _ _ famTcInj) => famTcInj
                                              | (Mk_PrimTyCon _ _ _ _ _ _ _ _ _) => (error
...
Definition famTcParent (__arg_69__ : TyCon) := (match __arg_69__ with
                                                 | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                 ...
                                                 | (Mk_FamilyTyCon _ _ _ _ _ _ _ _ _ famTcParent _) => famTcParent
                                                 | (Mk_PrimTyCon _ _ _ _ _ _ _ _ _) => (error
...
Definition famTcResVar (__arg_70__ : TyCon) := (match __arg_70__ with
                                                 | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                 ...
                                                 | (Mk_FamilyTyCon _ _ _ _ _ _ _ famTcResVar _ _ _) => famTcResVar
                                                 | (Mk_PrimTyCon _ _ _ _ _ _ _ _ _) => (error
...
Definition isUnlifted (__arg_71__ : TyCon) := (match __arg_71__ with
                                                | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                ...
                                                | (Mk_PrimTyCon _ _ _ _ _ _ _ isUnlifted _) => isUnlifted
                                                | (Mk_PromotedDataCon _ _ _ _ _ _ _ _ _ _) => (error
...
Definition primRepName (__arg_72__ : TyCon) := (match __arg_72__ with
                                                 | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                 ...
                                                 | (Mk_PrimTyCon _ _ _ _ _ _ _ _ primRepName) => primRepName
                                                 | (Mk_PromotedDataCon _ _ _ _ _ _ _ _ _ _) => (error
...
Definition promDcRepInfo (__arg_73__ : TyCon) := (match __arg_73__ with
                                                   | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                                                    &"Partial record selector: field `promDcRepInfo' has no match in constructor `Mk_FunTyCon' of type `TyCon'")
                                                   | (Mk_AlgTyCon _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) => (error
                                                                                                    &"Partial record selector: field `promDcRepInfo' has no match in constructor `Mk_AlgTyCon' of type `TyCon'")
                                                   | (Mk_SynonymTyCon _ _ _ _ _ _ _ _ _) => (error
                                                                                            &"Partial record selector: field `promDcRepInfo' has no match in constructor `Mk_SynonymTyCon' of type `TyCon'")
                                                   | (Mk_FamilyTyCon _ _ _ _ _ _ _ _ _ _ _) => (error
                                                                                               &"Partial record selector: field `promDcRepInfo' has no match in constructor `Mk_FamilyTyCon' of type `TyCon'")
                                                   | (Mk_PrimTyCon _ _ _ _ _ _ _ _ _) => (error
                                                                                         &"Partial record selector: field `promDcRepInfo' has no match in constructor `Mk_PrimTyCon' of type `TyCon'")
                                                   | (Mk_PromotedDataCon _ _ _ _ _ _ _ _ _ promDcRepInfo) =>
                                                     promDcRepInfo
                                                   | (Mk_TcTyCon _ _ _ _ _ _ _ _ _) => (error
                                                                                       &"Partial record selector: field `promDcRepInfo' has no match in constructor `Mk_TcTyCon' of type `TyCon'")
                                                 end).

Definition synTcRhs (__arg_74__ : TyCon) := (match __arg_74__ with
                                              | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                              ...
                                              | (Mk_SynonymTyCon _ _ _ _ _ _ _ _ synTcRhs) => synTcRhs
                                              | (Mk_FamilyTyCon _ _ _ _ _ _ _ _ _ _ _) => (error
...
Definition tcRepName (__arg_75__ : TyCon) := (match __arg_75__ with
                                               | (Mk_FunTyCon _ _ _ _ _ _ tcRepName) => tcRepName
                                               | (Mk_AlgTyCon _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) => (error
                                               ...
                                               | (Mk_PromotedDataCon _ _ _ _ _ _ _ _ tcRepName _) => tcRepName
                                               | (Mk_TcTyCon _ _ _ _ _ _ _ _ _) => (error
                                                                                   &"Partial record selector: field `tcRepName' has no match in constructor `Mk_TcTyCon' of type `TyCon'")
                                             end).

Definition tcRoles (__arg_76__ : TyCon) := (match __arg_76__ with
                                             | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                                              &"Partial record selector: field `tcRoles' has no match in constructor `Mk_FunTyCon' of type `TyCon'")
                                             | (Mk_AlgTyCon _ _ _ _ _ _ _ tcRoles _ _ _ _ _ _ _) => tcRoles
                                             | (Mk_SynonymTyCon _ _ _ _ _ _ _ tcRoles _) => tcRoles
                                             | (Mk_FamilyTyCon _ _ _ _ _ _ _ _ _ _ _) => (error
                                                                                         &"Partial record selector: field `tcRoles' has no match in constructor `Mk_FamilyTyCon' of type `TyCon'")
                                             | (Mk_PrimTyCon _ _ _ _ _ _ tcRoles _ _) => tcRoles
                                             | (Mk_PromotedDataCon _ _ _ _ _ _ tcRoles _ _ _) => tcRoles
                                             | (Mk_TcTyCon _ _ _ _ _ _ _ _ _) => (error
                                                                                 &"Partial record selector: field `tcRoles' has no match in constructor `Mk_TcTyCon' of type `TyCon'")
                                           end).

Definition tcTyConScopedTyVars (__arg_77__ : TyCon) := (match __arg_77__ with
                                                         | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                                                          &"Partial record selector: field `tcTyConScopedTyVars' has no match in constructor `Mk_FunTyCon' of type `TyCon'")
                                                         | (Mk_AlgTyCon _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) => (error
                                                                                                          &"Partial record selector: field `tcTyConScopedTyVars' has no match in constructor `Mk_AlgTyCon' of type `TyCon'")
                                                         | (Mk_SynonymTyCon _ _ _ _ _ _ _ _ _) => (error
                                                                                                  &"Partial record selector: field `tcTyConScopedTyVars' has no match in constructor `Mk_SynonymTyCon' of type `TyCon'")
                                                         | (Mk_FamilyTyCon _ _ _ _ _ _ _ _ _ _ _) => (error
                                                                                                     &"Partial record selector: field `tcTyConScopedTyVars' has no match in constructor `Mk_FamilyTyCon' of type `TyCon'")
                                                         | (Mk_PrimTyCon _ _ _ _ _ _ _ _ _) => (error
                                                                                               &"Partial record selector: field `tcTyConScopedTyVars' has no match in constructor `Mk_PrimTyCon' of type `TyCon'")
                                                         | (Mk_PromotedDataCon _ _ _ _ _ _ _ _ _ _) => (error
                                                                                                       &"Partial record selector: field `tcTyConScopedTyVars' has no match in constructor `Mk_PromotedDataCon' of type `TyCon'")
                                                         | (Mk_TcTyCon _ _ _ _ _ _ _ _ tcTyConScopedTyVars) =>
                                                           tcTyConScopedTyVars
                                                       end).

Definition tyConArity (__arg_78__ : TyCon) := (match __arg_78__ with
                                                | (Mk_FunTyCon _ _ _ _ _ tyConArity _) => tyConArity
                                                | (Mk_AlgTyCon _ _ _ _ _ tyConArity _ _ _ _ _ _ _ _ _) => tyConArity
                                                | (Mk_SynonymTyCon _ _ _ _ _ tyConArity _ _ _) => tyConArity
                                                | (Mk_FamilyTyCon _ _ _ _ _ tyConArity _ _ _ _ _) => tyConArity
                                                | (Mk_PrimTyCon _ _ _ _ _ tyConArity _ _ _) => tyConArity
                                                | (Mk_PromotedDataCon _ _ tyConArity _ _ _ _ _ _ _) => tyConArity
                                                | (Mk_TcTyCon _ _ _ _ _ _ _ tyConArity _) => tyConArity
                                              end).

Definition tyConBinders (__arg_79__ : TyCon) := (match __arg_79__ with
                                                  | (Mk_FunTyCon _ _ tyConBinders _ _ _ _) => tyConBinders
                                                  | (Mk_AlgTyCon _ _ tyConBinders _ _ _ _ _ _ _ _ _ _ _ _) =>
                                                    tyConBinders
                                                  | (Mk_SynonymTyCon _ _ tyConBinders _ _ _ _ _ _) => tyConBinders
                                                  | (Mk_FamilyTyCon _ _ tyConBinders _ _ _ _ _ _ _ _) => tyConBinders
                                                  | (Mk_PrimTyCon _ _ tyConBinders _ _ _ _ _ _) => tyConBinders
                                                  | (Mk_PromotedDataCon _ _ _ tyConBinders _ _ _ _ _ _) => tyConBinders
                                                  | (Mk_TcTyCon _ _ _ _ tyConBinders _ _ _ _) => tyConBinders
                                                end).

Definition tyConCType (__arg_80__ : TyCon) := (match __arg_80__ with
                                                | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                                                 &"Partial record selector: field `tyConCType' has no match in constructor `Mk_FunTyCon' of type `TyCon'")
                                                | (Mk_AlgTyCon _ _ _ _ _ _ _ _ tyConCType _ _ _ _ _ _) => tyConCType
                                                | (Mk_SynonymTyCon _ _ _ _ _ _ _ _ _) => (error
...
Definition tyConKind (__arg_81__ : TyCon) := (match __arg_81__ with
                                               | (Mk_FunTyCon _ _ _ _ tyConKind _ _) => tyConKind
                                               | (Mk_AlgTyCon _ _ _ _ tyConKind _ _ _ _ _ _ _ _ _ _) => tyConKind
                                               | (Mk_SynonymTyCon _ _ _ _ tyConKind _ _ _ _) => tyConKind
                                               | (Mk_FamilyTyCon _ _ _ _ tyConKind _ _ _ _ _ _) => tyConKind
                                               | (Mk_PrimTyCon _ _ _ _ tyConKind _ _ _ _) => tyConKind
                                               | (Mk_PromotedDataCon _ _ _ _ _ tyConKind _ _ _ _) => tyConKind
                                               | (Mk_TcTyCon _ _ _ _ _ _ tyConKind _ _) => tyConKind
                                             end).

Definition tyConName (__arg_82__ : TyCon) := (match __arg_82__ with
                                               | (Mk_FunTyCon _ tyConName _ _ _ _ _) => tyConName
                                               | (Mk_AlgTyCon _ tyConName _ _ _ _ _ _ _ _ _ _ _ _ _) => tyConName
                                               | (Mk_SynonymTyCon _ tyConName _ _ _ _ _ _ _) => tyConName
                                               | (Mk_FamilyTyCon _ tyConName _ _ _ _ _ _ _ _ _) => tyConName
                                               | (Mk_PrimTyCon _ tyConName _ _ _ _ _ _ _) => tyConName
                                               | (Mk_PromotedDataCon _ tyConName _ _ _ _ _ _ _ _) => tyConName
                                               | (Mk_TcTyCon _ tyConName _ _ _ _ _ _ _) => tyConName
                                             end).

Definition tyConResKind (__arg_83__ : TyCon) := (match __arg_83__ with
                                                  | (Mk_FunTyCon _ _ _ tyConResKind _ _ _) => tyConResKind
                                                  | (Mk_AlgTyCon _ _ _ tyConResKind _ _ _ _ _ _ _ _ _ _ _) =>
                                                    tyConResKind
                                                  | (Mk_SynonymTyCon _ _ _ tyConResKind _ _ _ _ _) => tyConResKind
                                                  | (Mk_FamilyTyCon _ _ _ tyConResKind _ _ _ _ _ _ _) => tyConResKind
                                                  | (Mk_PrimTyCon _ _ _ tyConResKind _ _ _ _ _) => tyConResKind
                                                  | (Mk_PromotedDataCon _ _ _ _ tyConResKind _ _ _ _ _) => tyConResKind
                                                  | (Mk_TcTyCon _ _ _ _ _ tyConResKind _ _ _) => tyConResKind
                                                end).

Definition tyConTyVars (__arg_84__ : TyCon) := (match __arg_84__ with
                                                 | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                                                  &"Partial record selector: field `tyConTyVars' has no match in constructor `Mk_FunTyCon' of type `TyCon'")
                                                 | (Mk_AlgTyCon _ _ _ _ _ _ tyConTyVars _ _ _ _ _ _ _ _) => tyConTyVars
                                                 | (Mk_SynonymTyCon _ _ _ _ _ _ tyConTyVars _ _) => tyConTyVars
                                                 | (Mk_FamilyTyCon _ _ _ _ _ _ tyConTyVars _ _ _ _) => tyConTyVars
                                                 | (Mk_PrimTyCon _ _ _ _ _ _ _ _ _) => (error
                                                 ...
                                                 | (Mk_TcTyCon _ _ _ tyConTyVars _ _ _ _ _) => tyConTyVars
                                               end).

Definition tyConUnique (__arg_85__ : TyCon) := (match __arg_85__ with
                                                 | (Mk_FunTyCon tyConUnique _ _ _ _ _ _) => tyConUnique
                                                 | (Mk_AlgTyCon tyConUnique _ _ _ _ _ _ _ _ _ _ _ _ _ _) => tyConUnique
                                                 | (Mk_SynonymTyCon tyConUnique _ _ _ _ _ _ _ _) => tyConUnique
                                                 | (Mk_FamilyTyCon tyConUnique _ _ _ _ _ _ _ _ _ _) => tyConUnique
                                                 | (Mk_PrimTyCon tyConUnique _ _ _ _ _ _ _ _) => tyConUnique
                                                 | (Mk_PromotedDataCon tyConUnique _ _ _ _ _ _ _ _ _) => tyConUnique
                                                 | (Mk_TcTyCon tyConUnique _ _ _ _ _ _ _ _) => tyConUnique
                                               end).

Definition tyConUnsat (__arg_86__ : TyCon) := (match __arg_86__ with
                                                | (Mk_FunTyCon _ _ _ _ _ _ _) => (error
                                                ...
                                                | (Mk_TcTyCon _ _ tyConUnsat _ _ _ _ _ _) => tyConUnsat
                                              end).

Definition data_con (__arg_87__ : AlgTyConRhs) := (match __arg_87__ with
...
Definition data_cons (__arg_88__ : AlgTyConRhs) := (match __arg_88__ with
                                                     | (Mk_AbstractTyCon _) => (error
...
Definition is_enum (__arg_89__ : AlgTyConRhs) := (match __arg_89__ with
                                                   | (Mk_AbstractTyCon _) => (error
...
Definition nt_co (__arg_90__ : AlgTyConRhs) := (match __arg_90__ with
                                                 | (Mk_AbstractTyCon _) => (error
...
Definition nt_etad_rhs (__arg_91__ : AlgTyConRhs) := (match __arg_91__ with
                                                       | (Mk_AbstractTyCon _) => (error
...
Definition nt_rhs (__arg_92__ : AlgTyConRhs) := (match __arg_92__ with
                                                  | (Mk_AbstractTyCon _) => (error
...
Definition tup_sort (__arg_93__ : AlgTyConRhs) := (match __arg_93__ with
                                                    | (Mk_AbstractTyCon _) => (error
...
Definition co_ax_branches {br} (__arg_94__ : (CoAxiom br)) :=
  (match __arg_94__ with
    | (Mk_CoAxiom _ _ _ _ co_ax_branches _) => co_ax_branches
  end).

Definition co_ax_implicit {br} (__arg_95__ : (CoAxiom br)) :=
  (match __arg_95__ with
    | (Mk_CoAxiom _ _ _ _ _ co_ax_implicit) => co_ax_implicit
  end).

Definition co_ax_name {br} (__arg_96__ : (CoAxiom br)) := (match __arg_96__ with
                                                            | (Mk_CoAxiom _ co_ax_name _ _ _ _) => co_ax_name
                                                          end).

Definition co_ax_role {br} (__arg_97__ : (CoAxiom br)) := (match __arg_97__ with
                                                            | (Mk_CoAxiom _ _ co_ax_role _ _ _) => co_ax_role
                                                          end).

Definition co_ax_tc {br} (__arg_98__ : (CoAxiom br)) := (match __arg_98__ with
                                                          | (Mk_CoAxiom _ _ _ co_ax_tc _ _) => co_ax_tc
                                                        end).

Definition co_ax_unique {br} (__arg_99__ : (CoAxiom br)) :=
  (match __arg_99__ with
    | (Mk_CoAxiom co_ax_unique _ _ _ _ _) => co_ax_unique
  end).

Definition unMkBranches {br : BranchFlag} (__arg_100__ : (Branches br)) :=
  (match __arg_100__ with
    | (Mk_MkBranches unMkBranches) => unMkBranches
  end).

Definition cab_cvs (__arg_101__ : CoAxBranch) := (match __arg_101__ with
                                                   | (Mk_CoAxBranch _ _ cab_cvs _ _ _ _) => cab_cvs
                                                 end).

Definition cab_incomps (__arg_102__ : CoAxBranch) := (match __arg_102__ with
                                                       | (Mk_CoAxBranch _ _ _ _ _ _ cab_incomps) => cab_incomps
                                                     end).

Definition cab_lhs (__arg_103__ : CoAxBranch) := (match __arg_103__ with
                                                   | (Mk_CoAxBranch _ _ _ _ cab_lhs _ _) => cab_lhs
                                                 end).

Definition cab_loc (__arg_104__ : CoAxBranch) := (match __arg_104__ with
                                                   | (Mk_CoAxBranch cab_loc _ _ _ _ _ _) => cab_loc
                                                 end).

Definition cab_rhs (__arg_105__ : CoAxBranch) := (match __arg_105__ with
                                                   | (Mk_CoAxBranch _ _ _ _ _ cab_rhs _) => cab_rhs
                                                 end).

Definition cab_roles (__arg_106__ : CoAxBranch) := (match __arg_106__ with
                                                     | (Mk_CoAxBranch _ _ _ cab_roles _ _ _) => cab_roles
                                                   end).

Definition cab_tvs (__arg_107__ : CoAxBranch) := (match __arg_107__ with
                                                   | (Mk_CoAxBranch _ cab_tvs _ _ _ _ _) => cab_tvs
                                                 end).

Definition idScope (__arg_108__ : Var) := (match __arg_108__ with
...
Definition id_details (__arg_109__ : Var) := (match __arg_109__ with
                                               | (Mk_TyVar _ _ _) => (error
...
Definition id_info (__arg_110__ : Var) := (match __arg_110__ with
                                            | (Mk_TyVar _ _ _) => (error
...
Definition realUnique (__arg_111__ : Var) := (match __arg_111__ with
                                               | (Mk_TyVar _ realUnique _) => realUnique
                                               | (Mk_TcTyVar _ realUnique _ _) => realUnique
                                               | (Mk_Id _ realUnique _ _ _ _) => realUnique
                                             end).

Definition tc_tv_details (__arg_112__ : Var) := (match __arg_112__ with
                                                  | (Mk_TyVar _ _ _) => (error
...
Definition sel_tycon (__arg_116__ : IdDetails) := (match __arg_116__ with
                                                    | Mk_VanillaId => (error
...
Definition dcEqSpec (__arg_117__ : DataCon) := (match __arg_117__ with
                                                 | (Mk_MkData _ _ _ _ _ _ _ _ dcEqSpec _ _ _ _ _ _ _ _ _ _ _ _ _ _) =>
                                                   dcEqSpec
                                               end).

Definition dcExTyBinders (__arg_118__ : DataCon) := (match __arg_118__ with
                                                      | (Mk_MkData _ _ _ _ _ _ _ dcExTyBinders _ _ _ _ _ _ _ _ _ _ _ _ _
                                                                   _ _) => dcExTyBinders
                                                    end).

Definition dcExTyVars (__arg_119__ : DataCon) := (match __arg_119__ with
                                                   | (Mk_MkData _ _ _ _ _ _ dcExTyVars _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
                                                                _) => dcExTyVars
                                                 end).

Definition dcFields (__arg_120__ : DataCon) := (match __arg_120__ with
                                                 | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ _ _ _ dcFields _ _ _ _ _ _ _ _) =>
                                                   dcFields
                                               end).

Definition dcInfix (__arg_121__ : DataCon) := (match __arg_121__ with
                                                | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ dcInfix _) =>
                                                  dcInfix
                                              end).

Definition dcName (__arg_122__ : DataCon) := (match __arg_122__ with
                                               | (Mk_MkData dcName _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) =>
                                                 dcName
                                             end).

Definition dcOrigArgTys (__arg_123__ : DataCon) := (match __arg_123__ with
                                                     | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ dcOrigArgTys _ _ _ _ _ _ _ _ _ _
                                                                  _) => dcOrigArgTys
                                                   end).

Definition dcOrigResTy (__arg_124__ : DataCon) := (match __arg_124__ with
                                                    | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ _ dcOrigResTy _ _ _ _ _ _ _ _ _
                                                                 _) => dcOrigResTy
                                                  end).

Definition dcOtherTheta (__arg_125__ : DataCon) := (match __arg_125__ with
                                                     | (Mk_MkData _ _ _ _ _ _ _ _ _ dcOtherTheta _ _ _ _ _ _ _ _ _ _ _ _
                                                                  _) => dcOtherTheta
                                                   end).

Definition dcPromoted (__arg_126__ : DataCon) := (match __arg_126__ with
                                                   | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
                                                                dcPromoted) => dcPromoted
                                                 end).

Definition dcRep (__arg_127__ : DataCon) := (match __arg_127__ with
                                              | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ dcRep _ _ _ _ _ _) => dcRep
                                            end).

Definition dcRepArity (__arg_128__ : DataCon) := (match __arg_128__ with
                                                   | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ dcRepArity _ _ _ _
                                                                _) => dcRepArity
                                                 end).

Definition dcRepTyCon (__arg_129__ : DataCon) := (match __arg_129__ with
                                                   | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ dcRepTyCon _ _
                                                                _) => dcRepTyCon
                                                 end).

Definition dcRepType (__arg_130__ : DataCon) := (match __arg_130__ with
                                                  | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ dcRepType _ _) =>
                                                    dcRepType
                                                end).

Definition dcSourceArity (__arg_131__ : DataCon) := (match __arg_131__ with
                                                      | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ dcSourceArity _ _
                                                                   _ _) => dcSourceArity
                                                    end).

Definition dcSrcBangs (__arg_132__ : DataCon) := (match __arg_132__ with
                                                   | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ _ _ dcSrcBangs _ _ _ _ _ _ _ _
                                                                _) => dcSrcBangs
                                                 end).

Definition dcStupidTheta (__arg_133__ : DataCon) := (match __arg_133__ with
                                                      | (Mk_MkData _ _ _ _ _ _ _ _ _ _ dcStupidTheta _ _ _ _ _ _ _ _ _ _
                                                                   _ _) => dcStupidTheta
                                                    end).

Definition dcTag (__arg_134__ : DataCon) := (match __arg_134__ with
                                              | (Mk_MkData _ _ dcTag _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) => dcTag
                                            end).

Definition dcUnique (__arg_135__ : DataCon) := (match __arg_135__ with
                                                 | (Mk_MkData _ dcUnique _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) =>
                                                   dcUnique
                                               end).

Definition dcUnivTyBinders (__arg_136__ : DataCon) := (match __arg_136__ with
                                                        | (Mk_MkData _ _ _ _ _ dcUnivTyBinders _ _ _ _ _ _ _ _ _ _ _ _ _
                                                                     _ _ _ _) => dcUnivTyBinders
                                                      end).

Definition dcUnivTyVars (__arg_137__ : DataCon) := (match __arg_137__ with
                                                     | (Mk_MkData _ _ _ _ dcUnivTyVars _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
                                                                  _) => dcUnivTyVars
                                                   end).

Definition dcVanilla (__arg_138__ : DataCon) := (match __arg_138__ with
                                                  | (Mk_MkData _ _ _ dcVanilla _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) =>
                                                    dcVanilla
                                                end).

Definition dcWorkId (__arg_139__ : DataCon) := (match __arg_139__ with
                                                 | (Mk_MkData _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ dcWorkId _ _ _ _ _ _ _) =>
                                                   dcWorkId
                                               end).

Definition dcr_arg_tys (__arg_140__ : DataConRep) := (match __arg_140__ with
...
Definition dcr_bangs (__arg_141__ : DataConRep) := (match __arg_141__ with
                                                     | Mk_NoDataConRep => (error
...
Definition dcr_boxer (__arg_142__ : DataConRep) := (match __arg_142__ with
                                                     | Mk_NoDataConRep => (error
...
Definition dcr_stricts (__arg_143__ : DataConRep) := (match __arg_143__ with
                                                       | Mk_NoDataConRep => (error
...
Definition dcr_wrap_id (__arg_144__ : DataConRep) := (match __arg_144__ with
                                                       | Mk_NoDataConRep => (error
...
Definition chCoercion (__arg_145__ : CoercionHole) := (match __arg_145__ with
                                                        | (Mk_CoercionHole _ chCoercion) => chCoercion
                                                      end).

Definition chUnique (__arg_146__ : CoercionHole) := (match __arg_146__ with
                                                      | (Mk_CoercionHole chUnique _) => chUnique
                                                    end).

Definition n_loc (__arg_147__ : Name) := (match __arg_147__ with
                                           | (Mk_Name _ _ _ n_loc) => n_loc
                                         end).

Definition n_occ (__arg_148__ : Name) := (match __arg_148__ with
                                           | (Mk_Name _ n_occ _ _) => n_occ
                                         end).

Definition n_sort (__arg_149__ : Name) := (match __arg_149__ with
                                            | (Mk_Name n_sort _ _ _) => n_sort
                                          end).

Definition n_uniq (__arg_150__ : Name) := (match __arg_150__ with
                                            | (Mk_Name _ _ n_uniq _) => n_uniq
                                          end).

Definition psArgs (__arg_151__ : PatSyn) := (match __arg_151__ with
                                              | (Mk_MkPatSyn _ _ psArgs _ _ _ _ _ _ _ _ _ _ _ _) => psArgs
                                            end).

Definition psArity (__arg_152__ : PatSyn) := (match __arg_152__ with
                                               | (Mk_MkPatSyn _ _ _ psArity _ _ _ _ _ _ _ _ _ _ _) => psArity
                                             end).

Definition psBuilder (__arg_153__ : PatSyn) := (match __arg_153__ with
                                                 | (Mk_MkPatSyn _ _ _ _ _ _ _ _ _ _ _ _ _ _ psBuilder) => psBuilder
                                               end).

Definition psExTyBinders (__arg_154__ : PatSyn) := (match __arg_154__ with
                                                     | (Mk_MkPatSyn _ _ _ _ _ _ _ _ _ _ psExTyBinders _ _ _ _) =>
                                                       psExTyBinders
                                                   end).

Definition psExTyVars (__arg_155__ : PatSyn) := (match __arg_155__ with
                                                  | (Mk_MkPatSyn _ _ _ _ _ _ _ _ _ psExTyVars _ _ _ _ _) => psExTyVars
                                                end).

Definition psFieldLabels (__arg_156__ : PatSyn) := (match __arg_156__ with
                                                     | (Mk_MkPatSyn _ _ _ _ _ psFieldLabels _ _ _ _ _ _ _ _ _) =>
                                                       psFieldLabels
                                                   end).

Definition psInfix (__arg_157__ : PatSyn) := (match __arg_157__ with
                                               | (Mk_MkPatSyn _ _ _ _ psInfix _ _ _ _ _ _ _ _ _ _) => psInfix
                                             end).

Definition psMatcher (__arg_158__ : PatSyn) := (match __arg_158__ with
                                                 | (Mk_MkPatSyn _ _ _ _ _ _ _ _ _ _ _ _ _ psMatcher _) => psMatcher
                                               end).

Definition psName (__arg_159__ : PatSyn) := (match __arg_159__ with
                                              | (Mk_MkPatSyn psName _ _ _ _ _ _ _ _ _ _ _ _ _ _) => psName
                                            end).

Definition psOrigResTy (__arg_160__ : PatSyn) := (match __arg_160__ with
                                                   | (Mk_MkPatSyn _ _ _ _ _ _ _ _ _ _ _ _ psOrigResTy _ _) =>
                                                     psOrigResTy
                                                 end).

Definition psProvTheta (__arg_161__ : PatSyn) := (match __arg_161__ with
                                                   | (Mk_MkPatSyn _ _ _ _ _ _ _ _ _ _ _ psProvTheta _ _ _) =>
                                                     psProvTheta
                                                 end).

Definition psReqTheta (__arg_162__ : PatSyn) := (match __arg_162__ with
                                                  | (Mk_MkPatSyn _ _ _ _ _ _ _ _ psReqTheta _ _ _ _ _ _) => psReqTheta
                                                end).

Definition psUnique (__arg_163__ : PatSyn) := (match __arg_163__ with
                                                | (Mk_MkPatSyn _ psUnique _ _ _ _ _ _ _ _ _ _ _ _ _) => psUnique
                                              end).

Definition psUnivTyBinders (__arg_164__ : PatSyn) := (match __arg_164__ with
                                                       | (Mk_MkPatSyn _ _ _ _ _ _ _ psUnivTyBinders _ _ _ _ _ _ _) =>
                                                         psUnivTyBinders
                                                     end).

Definition psUnivTyVars (__arg_165__ : PatSyn) := (match __arg_165__ with
                                                    | (Mk_MkPatSyn _ _ _ _ _ _ psUnivTyVars _ _ _ _ _ _ _ _) =>
                                                      psUnivTyVars
                                                  end).

Definition mtv_info (__arg_166__ : TcTyVarDetails) := (match __arg_166__ with
                                                        | (Mk_SkolemTv _) => (error
...
Definition mtv_ref (__arg_167__ : TcTyVarDetails) := (match __arg_167__ with
                                                       | (Mk_SkolemTv _) => (error
...
Definition mtv_tclvl (__arg_168__ : TcTyVarDetails) := (match __arg_168__ with
                                                         | (Mk_SkolemTv _) => (error
...
Definition envL (__arg_169__ : RnEnv2) := (match __arg_169__ with
                                            | (Mk_RV2 envL _ _) => envL
                                          end).

Definition envR (__arg_170__ : RnEnv2) := (match __arg_170__ with
                                            | (Mk_RV2 _ envR _) => envR
                                          end).

Definition in_scope (__arg_171__ : RnEnv2) := (match __arg_171__ with
                                                | (Mk_RV2 _ _ in_scope) => in_scope
                                              end).

Definition dv_kvs (__arg_172__ : TcDepVars) := (match __arg_172__ with
                                                 | (Mk_DV dv_kvs _) => dv_kvs
                                               end).

Definition dv_tvs (__arg_173__ : TcDepVars) := (match __arg_173__ with
                                                 | (Mk_DV _ dv_tvs) => dv_tvs
                                               end).

Definition tcm_covar {env} {m} (__arg_174__ : (TyCoMapper env m)) :=
  (match __arg_174__ with
    | (Mk_TyCoMapper _ _ tcm_covar _ _) => tcm_covar
  end).

Definition tcm_hole {env} {m} (__arg_175__ : (TyCoMapper env m)) :=
  (match __arg_175__ with
    | (Mk_TyCoMapper _ _ _ tcm_hole _) => tcm_hole
  end).

Definition tcm_smart {env} {m} (__arg_176__ : (TyCoMapper env m)) :=
  (match __arg_176__ with
    | (Mk_TyCoMapper tcm_smart _ _ _ _) => tcm_smart
  end).

Definition tcm_tybinder {env} {m} (__arg_177__ : (TyCoMapper env m)) :=
  (match __arg_177__ with
    | (Mk_TyCoMapper _ _ _ _ tcm_tybinder) => tcm_tybinder
  end).

Definition tcm_tyvar {env} {m} (__arg_178__ : (TyCoMapper env m)) :=
  (match __arg_178__ with
    | (Mk_TyCoMapper _ tcm_tyvar _ _ _) => tcm_tyvar
  end).

Definition df_args (__arg_179__ : Unfolding) := (match __arg_179__ with
...
Definition df_bndrs (__arg_180__ : Unfolding) := (match __arg_180__ with
                                                   | Mk_NoUnfolding => (error
...
Definition df_con (__arg_181__ : Unfolding) := (match __arg_181__ with
                                                 | Mk_NoUnfolding => (error
...
Definition uf_expandable (__arg_182__ : Unfolding) := (match __arg_182__ with
                                                        | Mk_NoUnfolding => (error
...
Definition uf_guidance (__arg_183__ : Unfolding) := (match __arg_183__ with
                                                      | Mk_NoUnfolding => (error
...
Definition uf_is_conlike (__arg_184__ : Unfolding) := (match __arg_184__ with
                                                        | Mk_NoUnfolding => (error
...
Definition uf_is_top (__arg_185__ : Unfolding) := (match __arg_185__ with
                                                    | Mk_NoUnfolding => (error
...
Definition uf_is_value (__arg_186__ : Unfolding) := (match __arg_186__ with
                                                      | Mk_NoUnfolding => (error
...
Definition uf_is_work_free (__arg_187__ : Unfolding) := (match __arg_187__ with
                                                          | Mk_NoUnfolding => (error
...
Definition uf_src (__arg_188__ : Unfolding) := (match __arg_188__ with
                                                 | Mk_NoUnfolding => (error
...
Definition uf_tmpl (__arg_189__ : Unfolding) := (match __arg_189__ with
                                                  | Mk_NoUnfolding => (error
...
Definition ru_act (__arg_190__ : CoreRule) := (match __arg_190__ with
...
Definition ru_args (__arg_191__ : CoreRule) := (match __arg_191__ with
...
Definition ru_auto (__arg_192__ : CoreRule) := (match __arg_192__ with
...
Definition ru_bndrs (__arg_193__ : CoreRule) := (match __arg_193__ with
...
Definition ru_fn (__arg_194__ : CoreRule) := (match __arg_194__ with
                                               | (Mk_Rule _ _ ru_fn _ _ _ _ _ _ _ _) => ru_fn
                                               | (Mk_BuiltinRule _ ru_fn _ _) => ru_fn
                                             end).

Definition ru_local (__arg_195__ : CoreRule) := (match __arg_195__ with
...
Definition ru_name (__arg_196__ : CoreRule) := (match __arg_196__ with
                                                 | (Mk_Rule ru_name _ _ _ _ _ _ _ _ _ _) => ru_name
                                                 | (Mk_BuiltinRule ru_name _ _ _) => ru_name
                                               end).

Definition ru_nargs (__arg_197__ : CoreRule) := (match __arg_197__ with
...
Definition ru_origin (__arg_198__ : CoreRule) := (match __arg_198__ with
...
Definition ru_orphan (__arg_199__ : CoreRule) := (match __arg_199__ with
...
Definition ru_rhs (__arg_200__ : CoreRule) := (match __arg_200__ with
...
Definition ru_rough (__arg_201__ : CoreRule) := (match __arg_201__ with
...
Definition ru_try (__arg_202__ : CoreRule) := (match __arg_202__ with
                                                | (Mk_Rule _ _ _ _ _ _ _ _ _ _ _) => (error
...
Definition re_base (__arg_203__ : RuleEnv) := (match __arg_203__ with
                                                | (Mk_RuleEnv re_base _) => re_base
                                              end).

Definition re_visible_orphs (__arg_204__ : RuleEnv) := (match __arg_204__ with
                                                         | (Mk_RuleEnv _ re_visible_orphs) => re_visible_orphs
                                                       end).

Definition inl_act (__arg_205__ : InlinePragma) := (match __arg_205__ with
                                                     | (Mk_InlinePragma _ _ _ inl_act _) => inl_act
                                                   end).

Definition inl_inline (__arg_206__ : InlinePragma) := (match __arg_206__ with
                                                        | (Mk_InlinePragma _ inl_inline _ _ _) => inl_inline
                                                      end).

Definition inl_rule (__arg_207__ : InlinePragma) := (match __arg_207__ with
                                                      | (Mk_InlinePragma _ _ _ _ inl_rule) => inl_rule
                                                    end).

Definition inl_sat (__arg_208__ : InlinePragma) := (match __arg_208__ with
                                                     | (Mk_InlinePragma _ _ inl_sat _ _) => inl_sat
                                                   end).

Definition inl_src (__arg_209__ : InlinePragma) := (match __arg_209__ with
