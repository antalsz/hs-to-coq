...
returnIO :: a -> IO a
returnIO x = undefined -- IO (\ s -> (# s, x #))

bindIO :: IO a -> (a -> IO b) -> IO b
bindIO (IO m) k = undefined -- IO (\ s -> case m s of (# new_s, a #) -> unIO (k a) new_s)

thenIO :: IO a -> IO b -> IO b
thenIO (IO m) k = undefined -- IO (\ s -> case m s of (# new_s, _ #) -> unIO k new_s)
